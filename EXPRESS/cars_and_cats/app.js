...
//fs is used to read folder contents, see how it is implemented below in /cars and /cats paths
const fs = require('fs');
...
//tell template engine that this is where the templates are going to live
...
app.get('/cars', (request, response) => {
    fs.readdir(path.join(__dirname, 'static/images/cars'), (error, cars) => {
        if (error) {
            throw (error);
        }     
        response.render('cars', {cars});
    });
});
